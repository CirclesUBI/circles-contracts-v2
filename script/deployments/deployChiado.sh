...
HUB=$(deploy_and_verify "Circles ERC1155 Hub" $HUB_ADDRESS_01 \
  src/hub/Hub.sol:Hub \
...
MIGRATION=$(deploy_and_verify "Migration" $MIGRATION_ADDRESS_02 \
  src/migration/Migration.sol:Migration \
  --constructor-args $V1_HUB_ADDRESS $HUB_ADDRESS_01)

NAME_REGISTRY=$(deploy_and_verify "NameRegistry" $NAMEREGISTRY_ADDRESS_03 \
  src/names/NameRegistry.sol:NameRegistry \
  --constructor-args $HUB_ADDRESS_01)

ERC20LIFT=$(deploy_and_verify "ERC20Lift" $ERC20LIFT_ADDRESS_04 \
...
STANDARD_TREASURY=$(deploy_and_verify "StandardTreasury" $STANDARD_TREASURY_ADDRESS_05 \
  src/treasury/StandardTreasury.sol:StandardTreasury \
  --constructor-args $HUB_ADDRESS_01 $MASTERCOPY_STANDARD_VAULT_09)

BASE_MINTPOLICY=$(deploy_and_verify "BaseGroupMintPolicy" $BASE_GROUPMINTPOLICY_ADDRESS_06 \
  src/groups/BaseMintPolicy.sol:MintPolicy)

MC_ERC20_DEMURRAGE=$(deploy_and_verify "MastercopyDemurrageERC20" $MASTERCOPY_DEMURRAGE_ERC20_ADDRESS_07 \
  src/lift/DemurrageCircles.sol:DemurrageCircles)

MC_ERC20_INFLATION=$(deploy_and_verify "MastercopyInflationaryERC20" $MASTERCOPY_INFLATIONARY_ERC20_ADDRESS_08 \
  src/lift/InflationaryCircles.sol:InflationaryCircles)

MC_STANDARD_VAULT=$(deploy_and_verify "MastercopyStandardVault" $MASTERCOPY_STANDARD_VAULT_09 \
...
# Define the summary file name with version
summary_file="chiado-$VERSION.log"

# Write the deployment details to the file
{
