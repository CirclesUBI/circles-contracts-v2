...
And then to account for the overcounting, with `K` and `L` the (in)completed hours

    - Γ^n * K - L
    = - R`(n) * K - L

So we need to create a new the table for `T'(n)`, but expect it to be identical to `T`:
...
and even more self-evident for R`:

```
n    GAMMA^n                                 64x64 Fixed (20 or 21 digits) 
---------------------------------------------------------------------------
0    1.0000000000000000000000000             18446744073709551616          
1    0.9998013320085989574306134             18443079296116538654          
2    0.9996027034861687221859511             18439415246597529027          
...
Which is a secondary (numerical) derivation of the same table, 
now in demurrage units, and we see that in the implementation,
we already computed the demurrage mint, times the inflationary factor `β^d`,
which we can now simply omit.